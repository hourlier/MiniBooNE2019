#ifndef FullOscClass_h
#define FullOscClass_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
 ...
 //MonteCarloBEAMChunkExt *MCBEAM_;
   ...
   FullOscClass(TTree *tree=0);
   virtual ~FullOscClass();
...
#ifdef FullOscClass_cxx
FullOscClass::FullOscClass(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("timing_MC_fullosc_may07_2.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("timing_MC_fullosc_may07_2.root");
      }
...
FullOscClass::~FullOscClass()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t FullOscClass::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t FullOscClass::LoadTree(Long64_t entry)
{
...
void FullOscClass::Init(TTree *tree)
{
...
void FullOscClass::Show(Long64_t entry)
{
...
Int_t FullOscClass::Cut(Long64_t entry)
{
