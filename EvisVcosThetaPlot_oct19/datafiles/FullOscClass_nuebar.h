#ifndef FullOscClass_nuebar_h
#define FullOscClass_nuebar_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
 ...
 //MonteCarloBEAMChunkExt *MCBEAM_;
   ...
   FullOscClass_nuebar(TTree *tree=0);
   virtual ~FullOscClass_nuebar();
   ...
   virtual void     Show(Long64_t entry = -1);
    ...
    void SetSin22theta(double s){sin22theta = s;}
    void SetDm2(double d){dm2 = d;}
    double getSin22theta(){return sin22theta;}
    double getdm2(){return dm2;}
};

#endif

#ifdef FullOscClass_nuebar_cxx
FullOscClass_nuebar::FullOscClass_nuebar(TTree *tree) : fChain(0)
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("timing2.0_MC_nuebar_fullosc_may07.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("timing2.0_MC_nuebar_fullosc_may07.root");
      }
...
FullOscClass_nuebar::~FullOscClass_nuebar()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t FullOscClass_nuebar::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t FullOscClass_nuebar::LoadTree(Long64_t entry)
{
...
void FullOscClass_nuebar::Init(TTree *tree)
{
...
Bool_t FullOscClass_nuebar::Notify()
{
...
void FullOscClass_nuebar::Show(Long64_t entry)
{
...
Int_t FullOscClass_nuebar::Cut(Long64_t entry)
{
...
#endif // #ifdef FullOscClass_nuebar_cxx
