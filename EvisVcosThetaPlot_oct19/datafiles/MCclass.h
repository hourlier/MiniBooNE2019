...
// Wed Jul 15 17:23:24 2020 by ROOT version 6.10/06
// from TTree TTiming/Timing Info
// found on file: timing_MC_nue_may07_2.root
//////////////////////////////////////////////////////////
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   static constexpr Int_t kMaxMCBEAM = 1;
    ...
    const int kkpmnue= 4;
    const int kmunue = 5;
    const int kElastic = 6;
    const int kNumu = 7;
    const int kCoherent = 8;

    int PlotColors[9];

    std::vector<TH1D*> hEnuqe;
    std::vector<TH1D*> hR_a;
    std::vector<TH1D*> hCosTheta;

    std::vector<TH2D*> hEvisSinTheta;
    std::vector<TH2D*> hEvisCosTheta;
    std::vector<TH2D*> hEvisCosTheta_09;
    std::vector<TH2D*> hEvisPperpComponents;
    TH2D *hEvisCosTheta_Coherent;

    std::vector<TH2D*> hREvis;
    std::vector<TH2D*> hXEvis;
    std::vector<TH2D*> hYEvis;
    std::vector<TH2D*> hZEvis;
    std::vector<TH2D*> hRnormEvis;

    std::vector<TH2D*> hDistToMeanCerEvis;
    std::vector<TH2D*> hFEvis;
    std::vector<TH2D*> hRendEvis;
    std::vector<TH2D*> hUdotREvis;
    std::vector<TH2D*> hX2Y2Evis;
    std::vector<TH2D*> hPperpEvis_Coherent;

    TH2D *hEvisPperp;
    TH1D *hBackgroundtype;
    TH1D *hR;
    TH1D *hMCEVNT_EVWT;
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("timing_MC_nue_may07_2.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("timing_MC_nue_may07_2.root");
      }
